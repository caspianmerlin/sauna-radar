...
use crate::{colour::Colour, SectorResult, error::Error, waypoint::{Airport, RunwayModifier, self, RunwayEnd, RunwayStrip, Vor, Ndb, Fix}, position::{Position, Heading, self}, AirspaceClass, sector::{Line, MultiLineMaybeColoured, MaybeColouredLine}};

use self::{sector_info::PartialSectorInfo, artcc::MultiLine};
...
pub struct PartialSector {
    ...
    ndbs: Vec<Ndb>,
    fixes: Vec<Fix>,
    artcc_entries: Vec<MultiLine>,
    artcc_low_entries: Vec<MultiLine>,
    artcc_high_entries: Vec<MultiLine>,
    low_airways: Vec<MultiLine>,
    high_airways: Vec<MultiLine>,
...
impl PartialSector {
    ...
    pub fn parse_colour_line(&mut self, value: &str) -> SectorResult<()> {
        ...
        let colour_name = sections.nth(1).ok_or(Error::InvalidColourDefinition)?.to_owned();
    ...
    pub fn parse_airport_line(&mut self, value: &str) -> SectorResult<()> {
        ...
        let airspace_class: AirspaceClass = sections.next().ok_or(Error::InvalidWaypoint)?.parse()?;
    ...
    pub fn parse_runway_line(&mut self, value: &str) -> SectorResult<()> {
        ...
        let heading_a = Heading::new(sections.next().ok_or(Error::InvalidRunway)?.parse::<f32>().map_err(|_|Error::InvalidRunway)?)?;
        let heading_b = Heading::new(sections.next().ok_or(Error::InvalidRunway)?.parse::<f32>().map_err(|_|Error::InvalidRunway)?)?;
        ...
        let airport = self.airports.iter_mut().find(|entry| entry.identifier == airport).ok_or(Error::InvalidRunway)?;
    ...
    pub fn parse_vor_or_ndb_line(&mut self, value: &str, beacon_type: BeaconType) -> SectorResult<()> {
        let mut sections = value.split_whitespace();
    ...
    pub fn parse_multi_line_line(&mut self, value: &str, artcc_entry_type: MultiLineType) -> SectorResult<()> {
        ...
        let lat = sections.next().ok_or(Error::InvalidFix)?;
        let lon = sections.next().ok_or(Error::InvalidFix)?;
        let pos_a = self.try_fetch_or_decode_lat_lon(lat, lon).ok_or(Error::InvalidArtccEntry)?;
        let lat = sections.next().ok_or(Error::InvalidFix)?;
        let lon = sections.next().ok_or(Error::InvalidFix)?;
        let pos_b = self.try_fetch_or_decode_lat_lon(lat, lon).ok_or(Error::InvalidArtccEntry)?;
        let line = Line {
            start: pos_a,
            end: pos_b,
        };
        let vec_to_push_to = match artcc_entry_type {
            MultiLineType::Artcc => &mut self.artcc_entries,
            MultiLineType::ArtccLow => &mut self.artcc_low_entries,
            MultiLineType::ArtccHigh => &mut self.artcc_high_entries,
            MultiLineType::LowAirway => &mut self.low_airways,
            MultiLineType::HighAirway => &mut self.high_airways,
        };

        if let Some(entry) = vec_to_push_to.iter_mut().find(|entry| entry.name == label) {
            entry.lines.push(line);
        } else {
            vec_to_push_to.push(MultiLine { name: label, lines: vec![line] });
        }

        Ok(())
    }

    pub fn parse_multi_line_maybe_coloured(&mut self, value: &str, sid_star_type: SidStarType) -> SectorResult<()> {
        let name = value.get(0..26).ok_or(Error::InvalidSidStarEntry)?.trim();
        let mut sections = value.get(26..).ok_or(Error::InvalidSidStarEntry)?.trim().split_whitespace();
        ...
        let colour = sections.next().and_then(|x| self.try_fetch_or_decode_colour(x));
        let line = self.try_fetch_or_decode_lat_lon(lat_a, lon_a).and_then(|start_pos| {
            self.try_fetch_or_decode_lat_lon(lat_b, lon_b).and_then(|end_pos| {
                Some(Line {
                    start: start_pos,
                    end: end_pos,
                })
        ...
        if name.is_empty() {
            ...
            let new_entry = MultiLineMaybeColoured {
                name: name.to_owned(),
                lines: if let Some(line) = line { vec![line] } else { vec![] },
            };
            vec.push(new_entry);
        }   
        Ok(())
